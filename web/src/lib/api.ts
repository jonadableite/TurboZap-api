...
const normalizeInstance = (raw: any): Instance => ({
  ...
  profileName: raw.profileName || raw.profile_name || raw.display_name || raw.name,
  profilePicture: raw.profilePicture || raw.profile_picture || raw.avatar || undefined,
  createdAt: raw.createdAt || raw.created_at || raw.created || new Date().toISOString(),
  updatedAt:
    raw.updatedAt || raw.updated_at || raw.updated || raw.createdAt || raw.created_at || new Date().toISOString(),
});

const normalizeInstanceArray = (payload?: any): Instance[] => {
  if (!payload) return [];
  if (Array.isArray(payload)) return payload.map(normalizeInstance);
  if (Array.isArray(payload.instances)) return payload.instances.map(normalizeInstance);
...
export const instanceApi = {
  ...
  create: async (data: CreateInstanceRequest): Promise<CreateInstanceResponse> => {
    const response = await api.post<CreateInstanceResponse>('/instance/create', data);
  ...
  getStatus: async (name: string): Promise<InstanceStatusResponse['data']> => {
    const response = await api.get<InstanceStatusResponse>(`/instance/${name}/status`);
