...
export default function DashboardPage() {
  ...
  return (
    <>
      <Header
        title="Dashboard"
        description="Visão geral do seu TurboZap"
      />
      ...
      <div className="px-4 sm:px-8 lg:px-14 py-8 space-y-8 max-w-6xl mx-auto w-full">
        ...
        >
          <div className="relative z-10">
            ...
            <p className="text-[var(--rocket-gray-300)] max-w-2xl mb-6">
              ...
              configure webhooks e muito mais com nossa API poderosa e fácil de usar.
            </p>
            <div className="flex items-center gap-4">
              <Button
                ...
                title={!canCreateInstance ? 'Configure sua API Key em Configurações' : undefined}
              >
                Nova instância
              </Button>
              <Link href="/instances">
            ...
            <svg viewBox="0 0 200 200" className="w-full h-full">
              <defs>
                <linearGradient id="grad" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" style={{ stopColor: 'var(--rocket-purple)' }} />
                  <stop offset="100%" style={{ stopColor: 'transparent' }} />
                </linearGradient>
              </defs>
              <circle cx="100" cy="100" r="80" fill="none" stroke="url(#grad)" strokeWidth="2" />
              <circle cx="100" cy="100" r="60" fill="none" stroke="url(#grad)" strokeWidth="1.5" />
              <circle cx="100" cy="100" r="40" fill="none" stroke="url(#grad)" strokeWidth="1" />
            </svg>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            <Card className="relative overflow-hidden">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-[var(--rocket-gray-400)]">Total Instâncias</p>
                  {isLoading ? (
                    <Spinner size="sm" />
                  ) : (
              ...
              <div className="mt-4 flex items-center gap-1 text-sm">
                <TrendingUp className="w-4 h-4 text-[var(--rocket-green)]" />
                <span className="text-[var(--rocket-green)]">+12%</span>
                <span className="text-[var(--rocket-gray-400)]">vs mês anterior</span>
              </div>
            </Card>
          </motion.div>
          ...
          >
            <Card className="relative overflow-hidden border-[var(--rocket-green)]/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-[var(--rocket-gray-400)]">Conectadas</p>
                  {isLoading ? (
                    <Spinner size="sm" />
                  ) : (
                ...
                <div className="w-12 h-12 rounded-xl bg-[var(--rocket-green)]/20 flex items-center justify-center">
                  <Activity className="w-6 h-6 text-[var(--rocket-green)]" />
                </div>
              </div>
              <div className="mt-4 flex items-center gap-2">
                <div className="flex-1 h-2 bg-[var(--rocket-gray-700)] rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: instances.length ? `${(connectedCount / instances.length) * 100}%` : '0%' }}
                    transition={{ delay: 0.5, duration: 0.8 }}
                    className="h-full bg-[var(--rocket-green)] rounded-full"
                  />
                </div>
                <span className="text-sm text-[var(--rocket-gray-400)]">
                  {instances.length ? Math.round((connectedCount / instances.length) * 100) : 0}%
                </span>
              </div>
            </Card>
          </motion.div>
          ...
          >
            <Card className="relative overflow-hidden border-[var(--rocket-danger)]/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-[var(--rocket-gray-400)]">Desconectadas</p>
                  {isLoading ? (
                    <Spinner size="sm" />
                  ) : (
                ...
                <div className="w-12 h-12 rounded-xl bg-[var(--rocket-danger)]/20 flex items-center justify-center">
                  <Clock className="w-6 h-6 text-[var(--rocket-danger)]" />
                </div>
              </div>
              {disconnectedCount > 0 && (
                <div className="mt-4">
          ...
          >
            <Card className="relative overflow-hidden">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-[var(--rocket-gray-400)]">Mensagens Hoje</p>
                  <p className="text-3xl font-bold text-[var(--rocket-gray-50)]">0</p>
                </div>
                <div className="w-12 h-12 rounded-xl bg-[var(--rocket-info)]/20 flex items-center justify-center">
                  <MessageSquare className="w-6 h-6 text-[var(--rocket-info)]" />
                </div>
              </div>
              <div className="mt-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Create Instance */}
            <motion.button
              onClick={() => canCreateInstance ? setShowCreateModal(true) : router.push('/settings')}
            ...
            >
              <div className="w-12 h-12 rounded-xl bg-[var(--rocket-green)]/20 flex items-center justify-center mb-4">
                <MessageSquare className="w-6 h-6 text-[var(--rocket-green)]" />
              </div>
                ...
                >
                  <Card className="group">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge
                        variant={instance.status === 'connected' ? 'success' : 'danger'}
